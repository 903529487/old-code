...
class LoginController extends ComController
{
    ...
    {
        $post=I('post.');
        $verify = isset($post['verify']) ? trim($post['verify']) : '';
        if (!$this->check_verify($verify, 'login')) {
            $this->error('验证码错误！', U("login/index"));
        }

        $username = isset($post['user']) ? trim($post['user']) : '';
        $password = isset($post['password']) ? password(trim($post['password'])) : '';
        $remember = isset($post['remember']) ? $post['remember'] : 0;
        if ($username == '') {
